...
class OmniglotDataLoader:
    def __init__(self, data_dir='./data', image_size=(20, 20), n_train_classses=126, n_test_classes=30):
        self.data = []
        ...
        for dirname, subdirname, filelist in os.walk(data_dir):
            if np.floor(100*count/280)%10==0 and count-break_pt>10:
                print(str(np.floor(100*count/280))+"% data loaded.")
                break_pt = count
            count = count+1
            if filelist:
                self.data.append(
